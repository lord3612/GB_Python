...
for elem in my_list:
    ...
    в надежде, что она не пропустит '+5', и попытаться как то иначе решить задачу, НО увы.
    И если увидел разок готовое решение, мозг отказывается искать другую логику."""
    if elem.isdigit():
...
было его вставлять? Делать в решинии ошибку ради того что бы ее в следствии исправить прибегая к различным ф-ям?
В общем мне пришло в голову только провести тот же алгоритм только 'без' пробела. Итог - работает"""
